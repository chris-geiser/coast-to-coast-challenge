...
- **Scale and burst behavior.** Every write goes through one global LockService lock and takes roughly 3.5 seconds, so the whole app tops out near 15 to 20 writes per minute. About 150 people logging a few times a day is comfortably under that. A launch-moment rush (many people logging in the same minute) is the one real pressure point: under it, Apps Script starts returning lock timeouts and, when overloaded, HTML error pages, and a write can even commit while its response is lost. The frontend is hardened for this: writes carry a client-generated entryId, the server replays that id idempotently instead of appending twice, delete is idempotent, and the client retries only transient failures (network error, non-JSON page, SERVER_ERROR) with backoff. Net effect: a burst mostly self-heals and cannot create duplicate miles. It can still feel slow during a spike.
